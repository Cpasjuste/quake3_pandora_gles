...
void Com_InitHunkMemory( void ) {
	...
	// cacheline align
	s_hunkData = (qbyte *) ( ( (int)s_hunkData + 31 ) & ~31 );
	Hunk_Clear();
...
void *Hunk_Alloc( int size, ha_pref preference ) {
...
#ifdef HUNK_DEBUG
	{
		...
		hunkblocks = block;
		buf = ((qbyte *) buf) + sizeof(hunkblock_t);
	}
#endif
	return buf;
}
...
int Com_EventLoop( void ) {
	...
	while ( 1 ) {
		...
		case SE_PACKET:
			...
			Com_Memcpy( buf.data, (qbyte *)((netadr_t *)ev.evPtr + 1), buf.cursize );
			if ( com_sv_running->integer ) {
				Com_RunAndTimeServerPacket( &evFrom, &buf );
			} else {
				CL_PacketEvent( evFrom, &buf );
...
skip4:							// 0-3 remaining bytes
		...
		mov		word ptr [edx],ax
		jl		outta
		mov		al,byte ptr [ebx+2]	// last qbyte
...
void Com_Memset (void* dest, const int val, const size_t count)
{
	...
	skip2:	cmp		ecx,0
			je		skip1
			mov		byte ptr [edx],al	// copy single qbyte
	skip1:
		}
		return;
	}
