...
static void LoadBMP( const char *name, qbyte **pic, int *width, int *height );
static void LoadTGA( const char *name, qbyte **pic, int *width, int *height );
static void LoadJPG( const char *name, qbyte **pic, int *width, int *height );

static qbyte			 s_intensitytable[256];
...
** R_GammaCorrect
*/
void R_GammaCorrect( qbyte *buffer, int bufSize ) {
	int i;
...
static void R_MipMap2( unsigned *in, int inWidth, int inHeight ) {
	...
	for ( i = 0 ; i < outHeight ; i++ ) {
		for ( j = 0 ; j < outWidth ; j++ ) {
			outpix = (qbyte *) ( temp + i * outWidth + j );
			for ( k = 0 ; k < 4 ; k++ ) {
				total = 
					1 * ((qbyte *)&in[ ((i*2-1)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask) ])[k] +
					2 * ((qbyte *)&in[ ((i*2-1)&inHeightMask)*inWidth + ((j*2)&inWidthMask) ])[k] +
					2 * ((qbyte *)&in[ ((i*2-1)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask) ])[k] +
					1 * ((qbyte *)&in[ ((i*2-1)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask) ])[k] +

					2 * ((qbyte *)&in[ ((i*2)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask) ])[k] +
					4 * ((qbyte *)&in[ ((i*2)&inHeightMask)*inWidth + ((j*2)&inWidthMask) ])[k] +
					4 * ((qbyte *)&in[ ((i*2)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask) ])[k] +
					2 * ((qbyte *)&in[ ((i*2)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask) ])[k] +

					2 * ((qbyte *)&in[ ((i*2+1)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask) ])[k] +
					4 * ((qbyte *)&in[ ((i*2+1)&inHeightMask)*inWidth + ((j*2)&inWidthMask) ])[k] +
					4 * ((qbyte *)&in[ ((i*2+1)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask) ])[k] +
					2 * ((qbyte *)&in[ ((i*2+1)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask) ])[k] +

					1 * ((qbyte *)&in[ ((i*2+2)&inHeightMask)*inWidth + ((j*2-1)&inWidthMask) ])[k] +
					2 * ((qbyte *)&in[ ((i*2+2)&inHeightMask)*inWidth + ((j*2)&inWidthMask) ])[k] +
					2 * ((qbyte *)&in[ ((i*2+2)&inHeightMask)*inWidth + ((j*2+1)&inWidthMask) ])[k] +
					1 * ((qbyte *)&in[ ((i*2+2)&inHeightMask)*inWidth + ((j*2+2)&inWidthMask) ])[k];
				outpix[k] = total / 36;
			}
		}
	}
...
==================
*/
static void R_BlendOverTexture( qbyte *data, int pixelCount, qbyte blend[4] ) {
	int		i;
	int		inverseAlpha;
	int		premult[3];
						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	else
	{
		...
		while ( width > scaled_width || height > scaled_height ) {
			R_MipMap( (qbyte *)data, width, height );
			width >>= 1;
			height >>= 1;
			if ( width < 1 ) {
				width = 1;
	...
	*format = internalFormat;
	...
	if (mipmap)
	{
		...
		{
			R_MipMap( (qbyte *)scaledBuffer, scaled_width, scaled_height );
			scaled_width >>= 1;
			scaled_height >>= 1;
			if (scaled_width < 1)
				scaled_width = 1;
			...
			if ( r_colorMipLevels->integer ) {
				R_BlendOverTexture( (qbyte *)scaledBuffer, scaled_width * scaled_height, mipBlendColors[miplevel] );
			}
...
================
*/
image_t *R_CreateImage( const char *name, const qbyte *pic, int width, int height, 
...
==============
*/
static void LoadPCX ( const char *filename, qbyte **pic, qbyte **palette, int *width, int *height)
{
	qbyte	*raw;
	pcx_t	*pcx;
	int		x, y;
	...
	*pic = out;
	...
	if (palette)
	{
		*palette = ri.Malloc(768);
		Com_Memcpy (*palette, (qbyte *)pcx + len - 768, 768);
	}
...
==============
*/
static void LoadPCX32 ( const char *filename, qbyte **pic, int *width, int *height) {
	qbyte	*palette;
	qbyte	*pic8;
	int		i, c, p;
	qbyte	*pic32;
...
void jpegDest (j_compress_ptr cinfo, qbyte* outfile, int size)
{
  my_dest_ptr dest;
...
=================
*/
void R_LoadImage( const char *name, qbyte **pic, int *width, int *height ) {
	int		len;
...
static void R_CreateDlightImage( void ) {
	int		x,y;
	qbyte	data[DLIGHT_SIZE][DLIGHT_SIZE][4];
	int		b;
	...
	tr.dlightImage = R_CreateImage("*dlight", (qbyte *)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse, GL_CLAMP );
}
...
static void R_CreateDefaultImage( void ) {
	int		x;
	qbyte	data[DEFAULT_SIZE][DEFAULT_SIZE][4];
	...
	tr.defaultImage = R_CreateImage("*default", (qbyte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qtrue, qfalse, GL_REPEAT );
}
...
void R_CreateBuiltinImages( void ) {
	...
	tr.whiteImage = R_CreateImage("*white", (qbyte *)data, 8, 8, qfalse, qfalse, GL_REPEAT );
	...
	tr.identityLightImage = R_CreateImage("*identityLight", (qbyte *)data, 8, 8, qfalse, qfalse, GL_REPEAT );


	for(x=0;x<32;x++) {
		// scratchimage is usually used for cinematic drawing
		tr.scratchImage[x] = R_CreateImage("*scratch", (qbyte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qfalse, qtrue, GL_CLAMP );
	}
