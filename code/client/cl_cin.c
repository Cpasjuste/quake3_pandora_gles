...
typedef struct {
	qbyte				linbuf[DEFAULT_CIN_WIDTH*DEFAULT_CIN_HEIGHT*4*2];
	qbyte				file[65536];
	short				sqrTable[256];

	unsigned int		mcomp[256];
	qbyte				*qStatus[2][32768];
...
typedef struct {
	...
	void ( *VQ0)(qbyte *status, void *qdata );
	void ( *VQ1)(qbyte *status, void *qdata );
	void ( *VQNormal)(qbyte *status, void *qdata );
	void ( *VQBuffer)(qbyte *status, void *qdata );
...
static void move8_32( qbyte *src, qbyte *dst, int spl )
{
	double *dsrc, *ddst;
	int dspl;
...
static void move4_32( qbyte *src, qbyte *dst, int spl  )
{
	double *dsrc, *ddst;
	int dspl;
...
static void blit8_32( qbyte *src, qbyte *dst, int spl  )
{
	double *dsrc, *ddst;
	int dspl;
...
******************************************************************************/
#define movs double
static void blit4_32( qbyte *src, qbyte *dst, int spl  )
{
	movs *dsrc, *ddst;
	int dspl;
...
static void blit2_32( qbyte *src, qbyte *dst, int spl  )
{
	double *dsrc, *ddst;
	int dspl;
...
static void blitVQQuad32fs( qbyte **status, unsigned char *data )
{
...
int		spl;
	...
	do {
		...
		switch (code) {
			case	0x8000:													// vq code
				blit8_32( (qbyte *)&vq8[(*data)*128], status[index], spl );
				data++;
				index += 5;
				break;
			case	0xc000:													// drop
				...
				for(i=0;i<4;i++) {
					...
					switch (code) {											// code in top two bits of code
						case	0x8000:										// 4x4 vq code
							blit4_32( (qbyte *)&vq4[(*data)*32], status[index], spl );
							data++;
							break;
						case	0xc000:										// 2x2 vq code
							blit2_32( (qbyte *)&vq2[(*data)*8], status[index], spl );
							data++;
							blit2_32( (qbyte *)&vq2[(*data)*8], status[index]+8, spl );
							data++;
							blit2_32( (qbyte *)&vq2[(*data)*8], status[index]+spl*2, spl );
							data++;
							blit2_32( (qbyte *)&vq2[(*data)*8], status[index]+spl*2+8, spl );
							data++;
							break;
...
static void decodeCodeBook( qbyte *input, unsigned short roq_flags )
{
	long	i, j, two, four;
	unsigned short	*aptr, *bptr, *cptr, *dptr;
	long	y0,y1,y2,y3,cr,cb;
	qbyte	*bbptr, *baptr, *bcptr, *bdptr;
...
//
			if (cinTable[currentHandle].samplesPerPixel==2) {
				...
				for(i=0;i<four;i++) {
					baptr = (qbyte *)vq2 + (*input++)*4;
					bbptr = (qbyte *)vq2 + (*input++)*4;
					for(j=0;j<2;j++) 
...
//
			if (cinTable[currentHandle].samplesPerPixel==2) {
				...
				for(i=0;i<four;i++) {
					baptr = (qbyte *)vq2 + (*input++)*8;
					bbptr = (qbyte *)vq2 + (*input++)*8;
					for(j=0;j<2;j++) {
						VQ2TO4(baptr,bbptr,bcptr,bdptr);
						VQ2TO4(baptr,bbptr,bcptr,bdptr);
					}
...
//
		if (cinTable[currentHandle].samplesPerPixel==2) {
			...
			for(i=0;i<four;i++) {
				baptr = (qbyte *)vq2 + (*input++)*2;
				bbptr = (qbyte *)vq2 + (*input++)*2;
				for(j=0;j<2;j++) { 
...
static void readQuadInfo( qbyte *qData )
{
...
static void initRoQ() 
{
	...
	cinTable[currentHandle].VQNormal = (void (*)(qbyte *, void *))blitVQQuad32fs;
	cinTable[currentHandle].VQBuffer = (void (*)(qbyte *, void *))blitVQQuad32fs;
...
******************************************************************************/
/*
static qbyte* RoQFetchInterlaced( qbyte *source ) {
	int x, *src, *dst;
	...
	switch(cinTable[currentHandle].roq_id) 
	{
		case	ROQ_QUAD_VQ:
			if ((cinTable[currentHandle].numQuads&1)) {
				...
				cinTable[currentHandle].VQ1( (qbyte *)cin.qStatus[1], framedata);
				...
				RoQPrepMcomp( cinTable[currentHandle].roqF0, cinTable[currentHandle].roqF1 );
				cinTable[currentHandle].VQ0( (qbyte *)cin.qStatus[0], framedata );
		...
		case	ZA_SOUND_MONO:
			if (!cinTable[currentHandle].silent) {
				ssize = RllDecodeMonoToStereo( framedata, sbuf, cinTable[currentHandle].RoQFrameSize, 0, (unsigned short)cinTable[currentHandle].roq_flags);
                                S_RawSamples( ssize, 22050, 2, 1, (qbyte *)sbuf, 1.0f );
			}
			break;
		case	ZA_SOUND_STEREO:
			if (!cinTable[currentHandle].silent) {
				...
				ssize = RllDecodeStereoToStereo( framedata, sbuf, cinTable[currentHandle].RoQFrameSize, 0, (unsigned short)cinTable[currentHandle].roq_flags);
                                S_RawSamples( ssize, 22050, 2, 2, (qbyte *)sbuf, 1.0f );
			}
			break;
		case	ROQ_QUAD_INFO:
			if (cinTable[currentHandle].numQuads == -1) {
...
void CIN_DrawCinematic (int handle) {
	...
	if (cinTable[handle].dirty && (cinTable[handle].CIN_WIDTH != cinTable[handle].drawX || cinTable[handle].CIN_HEIGHT != cinTable[handle].drawY)) {
		...
		re.DrawStretchRaw( x, y, w, h, 256, 256, (qbyte *)buf2, handle, qtrue);
